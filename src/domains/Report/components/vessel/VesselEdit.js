...
import {
    GetVesselTypeAction, GetCountryDataAction, UpdateVessel,
    VesselEmptyEditMessage, GetVesselDetails
} from "../../_redux/actions/VesselAction";
// import "./css/style.css";
import FileBase64 from "react-file-base64";
import { RHFInput } from "react-hook-form-input";

const VesselEdit = withRouter(({ history, props }) => {
    const { register, handleSubmit, errors, setValue } = useForm();
    const dispatch = useDispatch();
    const [loading, setLoading] = useState(false);
    const [files, setFiles] = useState([]);
    toast.configure();
    const { intID, intTotalCrew, numDeadWeight, numNetWeight, numGrossWeight, strBuildYear, strEngineName, strVesselFlag, strVesselName, intVesselTypeID, intYardCountryId, strVesselTypeName, strYardCountryName, strIMONumber } = props.location.state.vessel
